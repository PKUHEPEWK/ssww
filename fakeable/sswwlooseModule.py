...
class sswwlooseProducer(Module):
    ...
    def analyze(self, event):
        ...
        if (len(loose_leptons) > self.maxObjects or len(loose_leptons) < 2):
            return False

        tauVeto = 0
        for i in range(0,len(taus)):
            if (taus[i].pt > 18 and abs(taus[i].eta) < 2.3 and taus[i].idDecayMode and taus[i].idDecayModeNewDMs and taus[i].rawIso < 5 and deltaR(leptons[loose_leptons[0]][0].eta,leptons[loose_leptons[0]][0].phi,taus[i].eta,taus[i].phi) > 0.4 and deltaR(leptons[loose_leptons[1]][0].eta,leptons[loose_leptons[1]][0].phi,taus[i].eta,taus[i].phi) > 0.4):
                tauVeto = 1
        ...
        self.out.fillBranch("lepton_wp",2) # 0 means tight, 1 means fakeable, 2 means loose
        ...
        if lepton_pt[0] > -99. and lepton_pt[1] > -99.:
            self.out.fillBranch("mll",(leptons[loose_leptons[0]][0].p4() + leptons[loose_leptons[1]][0].p4()).M())
        else:
            self.out.fillBranch("mll",-99.)

        if lepton_pt[0] > -99. and lepton_pt[2] > -99.:
            self.out.fillBranch("mll02",(leptons[loose_leptons[0]][0].p4() + leptons[loose_leptons[2]][0].p4()).M())
        else:
            self.out.fillBranch("mll02",-99.)

        if lepton_pt[1] > -99. and lepton_pt[2] > -99.:
            self.out.fillBranch("mll12",(leptons[loose_leptons[1]][0].p4() + leptons[loose_leptons[2]][0].p4()).M())
        else:
        ...
        if leptons[loose_leptons[0]][0].pdgId == 11:
            self.out.fillBranch("mt",sqrt(2*leptons[loose_leptons[0]][0].pt/leptons[loose_leptons[0]][0].eCorr*event.MET_pt*(1 - cos(event.MET_phi - leptons[loose_leptons[0]][0].phi))))
        else:
        ...
        if leptons[loose_leptons[1]][0].pdgId == 11:
            self.out.fillBranch("mt1",sqrt(2*leptons[loose_leptons[1]][0].pt/leptons[loose_leptons[1]][0].eCorr*event.MET_pt*(1 - cos(event.MET_phi - leptons[loose_leptons[1]][0].phi))))
        else:
            self.out.fillBranch("mt1",sqrt(2*leptons[loose_leptons[1]][0].pt*event.MET_pt*(1 - cos(event.MET_phi - leptons[loose_leptons[1]][0].phi))))

        self.out.fillBranch("puppimet",event.PuppiMET_pt)

        if abs(jets[tight_jets[0]].eta - jets[tight_jets[1]].eta) > 0.:
            self.out.fillBranch("zepp0",abs((leptons[loose_leptons[0]][0].eta-(jets[tight_jets[0]].eta+jets[tight_jets[1]].eta)/2.)/(jets[tight_jets[0]].eta - jets[tight_jets[1]].eta)))
        else:
            self.out.fillBranch("zepp0",-99.)

        if abs(jets[tight_jets[0]].eta - jets[tight_jets[1]].eta) > 0.:
            self.out.fillBranch("zepp1",abs((leptons[loose_leptons[1]][0].eta-(jets[tight_jets[0]].eta+jets[tight_jets[1]].eta)/2.)/(jets[tight_jets[0]].eta - jets[tight_jets[1]].eta)))
        else:
            self.out.fillBranch("zepp1",-99.)

        return True

sswwlooseModule = lambda : sswwlooseProducer(maxObjects=3)
sswwlooseModuleData = lambda : sswwlooseProducer(maxObjects=3,isData = True)
